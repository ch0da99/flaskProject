    ...
    if request.method == "POST":
        ...
        if p_item_object:
            if current_user.can_purchase(p_item_object):
                ...
                flash(f"Unfortunately, you don't have enough money to "
                      f"purchase {p_item_object.name}.", category='danger')
        # Sell Item Logic
        ...
        if s_item_object:
            if current_user.can_sell(s_item_object):
                ...
                flash(f'Congratulations! You sold {s_item_object.name}'
                      f' back to market', category='success')
            else:
                flash(f"Unfortunately, something went wrong with"
                      f" selling an {s_item_object.name}", category='danger')
